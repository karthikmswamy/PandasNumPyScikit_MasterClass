...
def predict_data(input_data: dict) -> dict:
    ...
    [
        label_encoder_new,
        std_scaler_new,
        random_forest_classifier_new,
        random_forest_regressor_new,
    ] = joblib.load("./multi_output_model.pkl")
    test_df_scaled = std_scaler_new.transform(test_df)
    y_pred_style = label_encoder_new.inverse_transform(
        random_forest_classifier_new.predict(test_df_scaled)
    )
    y_pred_abv = random_forest_regressor_new.predict(test_df_scaled)

    results = {"Style": y_pred_style[0], "ABV": y_pred_abv[0]}
